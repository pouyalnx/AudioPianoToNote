...
N_note=0
with open("note_part.txt") as fnote:
    while True:
        line=fnote.readline()
        if line=="":
            break
...
def note2f(note,rep:int):
    return 2.5*NOTE_FREQ_TABLE[note]
    ...
    while True:
        ...
        for it in cmd:
            ...
            for rep in range(N_note_echo):
                f=note2f(note,rep)
                a=noteAmpRep(rep)
                data[rep_ptr:rep_ptr+L_note]+=a*np.sin(index*2*f*np.pi)+a*0.6*np.sin(index*2*2*f*np.pi)+a*0.2*np.sin(index*2*4*f*np.pi)+a*0.06*np.sin(index*2*8*f*np.pi)
                rep_ptr+=L_note
            ################################################################
        N_ptr+=1
        L_ptr+=L_note
